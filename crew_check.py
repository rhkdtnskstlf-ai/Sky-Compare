...
st.markdown("""
    ...
    .item-container { display: flex; align-items: center; padding: 10px 14px; margin: 8px 0; border-radius: 8px; font-size: 1rem; line-height: 1.5; font-weight: 500; }
...
if df_l is not None and df_r is not None:
    ...
    st.divider()
    view_l, view_center, view_r = st.columns([1, 1.8, 1])
    ...
    def display_list(container, df, title):
        with container:
            ...
            for flt in df["Arr Flt"].unique():
                ...
                disp = gp.copy()
                disp["연박"] = disp["is_layover"].map({True: "✅", False: ""})
                disp["Crew"] = disp["CrewName"] + " (" + disp["CrewID"] + ")"
                disp["시간"] = disp["Arr Time"].apply(format_time_display)
                st.table(disp[["연박", "시간", "Crew"]])
    ...
    with view_center:
        ...
        for flt in all_flts_ordered:
            ...
            for r in rem_list:
                items_html.append(f"<div class='item-container bg-out'><span class='badge badge-out'>OUT</span> {r['CrewName']}</div>")
            for a in add_list:
    ...
    st.success("✅ 분석 완료: 시간순 정렬이 반영되었습니다.")
else:
    st.info("💡 파일을 업로드하면 시간순으로 정렬하여 분석을 시작합니다.")
